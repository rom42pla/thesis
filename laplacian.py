...
def inference_on_dataset(model, dataloader_support, dataloader_query, evaluator,
    ...
    with inference_context(model), torch.no_grad():
        for img_data in dataloader_support.dataset.dataset:
            img = img_data["image"]  # torch.Size([3, H, W])
            boxes, box_labels = img_data["instances"].get_fields()["gt_boxes"].tensor.int(), \
                                img_data["instances"].get_fields()["gt_classes"]
            for box, label in zip(boxes, box_labels):
                ...
                features = resnet18(img_crop.unsqueeze(0)).flatten().to("cpu")
                # updates the embeddings
                ...
                for k, v in output.items():
                    ...
                    if isinstance(v, Instances):
                        ...
                        for i_box, box in enumerate(boxes):
                            ...
                            box_features += [resnet18(img_crop.unsqueeze(0)).flatten()]
    ...
    # save_results(join(".", "results"),
    #              inputs=inputs_total, outputs=outputs_total)
    # print(inputs)
    labels = laplacian_shot(X_s_embeddings=torch.stack(features_support),
                            X_s_labels=torch.stack(labels_support),
                            X_q_embeddings=torch.stack([b for s in outputs_total
                                                        for b in s["instances"].get_fields()["box_features"]]))

    # evaluates the results
    for input, output in zip(inputs_total, outputs_total):
...
def laplacian_shot(X_s_embeddings: torch.Tensor, X_s_labels: torch.Tensor,
                   ...
                   lambda_factor: float = 0.2) -> torch.Tensor:
