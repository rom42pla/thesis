...
def run_exp(cfg, configs):
    ...
    #     train_cmd = 'python -m tools.train_net --dist-url auto --num-gpus {} ' \
    #                 '--config-file {} --resume'.format(args.num_gpus, cfg)
    #     # train_cmd = 'python -m tools.train_net_laplacian --dist-url auto --num-gpus {} ' \
    #     #             '--config-file {} --resume --eval_during_training'.format(args.num_gpus, cfg)
    #     run_cmd(train_cmd)
    ...
    if not os.path.exists(res_path):
        # test_cmd = 'python -m tools.test_net_laplacian --dist-url auto --num-gpus {} ' \
            # '--config-file {} --resume --eval-only'.format(args.num_gpus, cfg)
        test_cmd = 'python -m tools.test_net_laplacian --dist-url auto --num-gpus {} ' \
                   '--config-file {} --resume --eval-during-train'.format(args.num_gpus, cfg)
        run_cmd(test_cmd)
